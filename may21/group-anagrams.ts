function groupAnagrams(strs: string[]): string[][] {
    ...
    */
    ...
    const maps: [number[], string][]= new Array(N).fill(0).map(()=>[new Array(26).fill(0),""]);
    for(let i=0;i<N;i++){
        const word=strs[i];
        maps[i][1]=word;
        for(const c of word){
    ...
    for(let i=1;i<N;i++){
      ...
      if(compare(maps[i],maps[i-1])===0){
        ...
        else{ //else add the previous group to the output, start a new group, and add it to new group
            out.push(currGroup);
            currGroup=[maps[i][1]];
        }   
    }
